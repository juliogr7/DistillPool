...
class UNet(Denoiser):
    ...
    def forward(self, x, sigma=None, return_features=False, **kwargs):
        r"""
        Run the denoiser on noisy image. The noise level is not used in this denoiser.

        :param torch.Tensor x: noisy image.
        :param float sigma: noise level (not used).
        :param bool return_features: return intermediate features.
        ...
        if x.size(2) % factor == 0 and x.size(3) % factor == 0:
            return self._forward(
                x, return_features=return_features, **kwargs
            )  # ,  **kwargs
        else:
            forward_fn = lambda x: self._forward(
                x, return_features=return_features, **kwargs
            )
            return test_pad(forward_fn, x, modulo=factor, **kwargs)  # ,  **kwargs

    def forward_standard(self, x, get_latent=False, return_features=False):
        # encoding path
        cat_dim = 1
        input = x
        x1 = self.Conv1(input)
        ...
        # step1: add a new keyword return_features
        if return_features:
    ...
    def forward_compact4(self, x, return_features=False):
        ...
        if return_features:
            features = [x1, x2, x3, x4, d4, d3, d2, d1]
            return out, features
        else:
            return out

    def forward_compact3(self, x, return_features=False):
        # encoding path
        cat_dim = 1
        input = x
        x1 = self.Conv1(input)
    ...
    def forward_compact2(self, x, return_features=False):
        # encoding path
        cat_dim = 1
        input = x
        x1 = self.Conv1(input)
        ...
        if return_features:
            features = [x1, x2, d2, d1]
            return out, features
        else:
            return out

    def load_weights(self, weights_dir, device="cpu"):
        """
        Carga los pesos del modelo desde un archivo.

        :param str weights_dir: Ruta al archivo de pesos.
        """
...
class HybridUNet(nn.Module):
    def __init__(self, encoder: UNet, decoder: UNet):
        super(HybridUNet, self).__init__()

        # === COPY ENCODER FROM STUDENT ===
        ...
        self.cat = encoder.cat
        self.residual = encoder.residual
        self.in_channels = encoder.in_channels
        self.out_channels = decoder.out_channels  # From decoder
        self.compact = encoder.compact  # number of scales

        # === COPY DECODER FROM TEACHER ===
        self.Up5 = decoder.Up5
        self.Up_conv5 = decoder.Up_conv5
        self.Up4 = decoder.Up4
        self.Up_conv4 = decoder.Up_conv4
        self.Up3 = decoder.Up3
        self.Up_conv3 = decoder.Up_conv3
        self.Up2 = decoder.Up2
        self.Up_conv2 = decoder.Up_conv2
        self.Conv_1x1 = decoder.Conv_1x1
    ...
    def forward(self, x, return_features=False):
        ...
        x3 = self.Maxpool(x2)
        x3 = self.Conv3(x3) if self.compact >= 3 else None

        x4 = self.Maxpool(x3) if self.compact >= 4 else None
        x4 = self.Conv4(x4) if self.compact >= 4 else None

        x5 = self.Maxpool(x4) if self.compact == 5 else None
        x5 = self.Conv5(x5) if self.compact == 5 else None

        if self.compact == 5:
            d5 = self.Up5(x5)
            if self.cat: d5 = self.Up_conv5(torch.cat((x4, d5), dim=cat_dim))
            d4 = self.Up4(d5)
            if self.cat: d4 = self.Up_conv4(torch.cat((x3, d4), dim=cat_dim))
            d3 = self.Up3(d4)
            if self.cat: d3 = self.Up_conv3(torch.cat((x2, d3), dim=cat_dim))
            d2 = self.Up2(d3)
            if self.cat: d2 = self.Up_conv2(torch.cat((x1, d2), dim=cat_dim))
