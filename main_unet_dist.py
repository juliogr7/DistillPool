...
def main(args):
    ...
    args.sup_loss = bool(args.sup_loss)
    print(args)
    ...
    config = (f"Distillation_UNET_crs_{str(args.cr_student).replace('.', '')}_crt_{str(args.cr_teacher).replace('.', '')}" +
                f"_alpha_{str(args.alpha).replace('.', '')}"
                f"_scales_{args.student_scales}_type_{type_dist}"
                f"{awgn_str}"
                f"_{date}")
    ...
    if not args.sup_loss:
        print(f"Distillation type: {type_dist} and with unsupervised loss")
    else:
    ...
    undersampling_rate = physics_s.mask.mean()
    ...
    if expert_t:
        # os.environ["EXPERT_TEACHER_PATH"] = "path.pth"
        filename = os.getenv("EXPERT_TEACHER_PATH", None)
        print('Distillation with EXPERT teacher!!!')
    else:
    ...
    for iter in range(args.iters):
        ...
        psnr_teacher_meter = AverageMeter()
        ...
        # for batch_idx, images in data_loop_train:
        for data in data_loop_train:
            # ojo: es para pruebas cortas
                # if i >=4:
                # break
            ...
            with torch.no_grad():
                ...
                psnr_teacher = psnr(teacher_output, images)
            ...
            total_loss = (beta * loss_gt) + (alpha * distill_loss)
            ...
            psnr_teacher_meter.update(psnr_teacher.item(), images.size(0))
            ...
            postfix_dict = {
                ...
                "psnr": psnr_meter.avg,
                "psnr_teacher": psnr_teacher_meter.avg,
            }
        ...
        if iter == 200:
            print("Checkpoint: Saving student model at iteration 200")
            alpha_str = str(args.alpha).replace('.', '')
            beta_str = str(args.beta).replace('.', '')
            filename = (f"{date}_student_weights_crs_{str(args.cr_student).replace('.', '')}_crt_{str(args.cr_teacher).replace('.', '')}"
                        f"_scales_{args.student_scales}_alpha_{alpha_str}_beta_{beta_str}_type_{type_dist}_"
                        f"loss_dist_{args.loss_distill}_loss_gt_{args.loss_gt}_iters_{args.iters}"
                        f"{awgn_str}"
                        f"dataset_{args.dataset}_" + "_at_iter_200.pth")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--sup_loss", type=int, default=1, help="Supervised loss (True) or unsupervised loss (False)"
    )
    parser.add_argument(
        "--cr_teacher",
        type=float,
    ...
    parser.add_argument("--beta", type=float, default=1, help="Weight of MSE Loss. Between [0,1]") # NO TOCAR
