    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.fast:
            ...
            revi = get_permutation_list(H)
            revj = get_permutation_list(W)

            indx = torch.arange(0, H * W)
            mask = indx < m
            mask = mask.reshape(H, W).repeat(C, 1, 1).unsqueeze(0).float()

            revi = np.argsort(revi)
            mask = mask.index_select(-2, torch.tensor(revi, device=mask.device))

            revj = np.argsort(revj)
            mask = mask.index_select(-1, torch.tensor(revj, device=mask.device))
